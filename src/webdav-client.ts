...
export class WebDAVClient {
    ...
    /**
     * Stream file content for hashing (returns async iterator of chunks)
     */
    async *streamFile(filePath: string): AsyncGenerator<Buffer> {
        const url = this.toWebDAVUrl(filePath);
        ...
        if (!response.body) {
            throw new Error(`WebDAV GET returned no body for ${filePath}`);
        }
